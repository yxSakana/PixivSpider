...
class PixivSettingUi(QWidget):
    def __init__(self, pixiv: Pixiv, parent: QWidget | None = None):
        ...
        self.content_edit_config = QLineEdit(os.path.abspath(self.pixiv.config_filename), self)
        ...
        self.content_edit_download_path = QLineEdit(self.pixiv.base_download_path, self)
        ...
        with open(self.pixiv.config_filename, "r") as f:
            _filename = os.path.abspath(json.loads(f.read())["spider"]["cookies_pools"][0])
        self.content_edit_cookie_files = QLineEdit(_filename, self)
        self.pb_cookie_files = QPushButton(QIcon("resource/cookie.png"), "", self)
        self.text_edit_cookie_files = QTextEdit(self.pixiv.header["cookie"], self)
    ...
    def onSelectDownloadPath(self):
        ...
        if result:
            ...
            print(self.pixiv.config_filename)
            with open(self.pixiv.config_filename, "r", encoding="utf-8") as file:
                data = json.loads(file.read())
            data["spider"]["base_path"] = result
            data["spider"]["base_download_path"] = result
            with open(self.pixiv.config_filename, "w", encoding="utf-8") as file:
    ...
    def onSelectCookieFiles(self):
        ...
        if result:
            with open(self.pixiv.config_filename, "r", encoding="utf-8") as file:
                data = json.loads(file.read())
            data["spider"]["cookies_pools"][0] = result[0]
            with open(self.pixiv.config_filename, "w", encoding="utf-8") as file:
                file.write(json.dumps(data, indent=4, ensure_ascii=False))
            self.pixiv.reloadConfig()
            self.pixiv.header["cookie"] = self.pixiv.cookies_pools[0]
            self.text_edit_cookie_files.setText(self.pixiv.header["cookie"])
